...
e2eTest.describe("Active Workout", () => {
  ...
  e2eTest(
    "should show start new workout button when no active session",
    async ({ workoutPage }) => {
      await workoutPage.waitForConvexData();
      const hasNoSession = await workoutPage.hasNoActiveSession();
      if (hasNoSession) {
  ...
  e2eTest(
    "should navigate back to dashboard from empty state",
  ...
  e2eTest(
    "should open new session modal from empty state",
    async ({ workoutPage, page }) => {
      await workoutPage.waitForConvexData();
      const hasNoSession = await workoutPage.hasNoActiveSession();
      if (hasNoSession) {
  ...
  e2eTest(
    ...
    async ({ workoutPage, dashboardPage }) => {
  ...
  e2eTest(
    "should be able to close modal without creating session",
    async ({ workoutPage, page }) => {
      await workoutPage.waitForConvexData();
      const hasNoSession = await workoutPage.hasNoActiveSession();
      if (hasNoSession) {
  ...
  e2eTest(
    "should resume active session from dashboard",
