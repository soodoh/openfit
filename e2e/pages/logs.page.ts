...
export class LogsPage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        this.pageSubtext = page.getByText(/track your progress and review past sessions/i);
        // Calendar elements
        this.calendar = page
    ...
    async isNewSessionModalOpen(): Promise<boolean> {
        return this.isVisible(this.page.getByRole("dialog").filter({ hasText: /session|workout/i }));
    }
    /**
    ...
    async getDaysWithSessions(): Promise<number[]> {
        ...
        const indicators = this.page.locator("[class*='indicator'], [class*='dot'], [class*='session']");
