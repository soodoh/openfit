...
export class WorkoutPage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        this.noActiveSessionMessage = page.getByText(/start a new workout session to begin tracking/i);
    ...
    async waitForWorkoutPageReady(): Promise<void> {
        ...
        await expect(this.page.locator("text=/no active workout|session|workout/i").first()).toBeVisible({ timeout: 15_000 });
    }
    /**
    ...
    async isFinishConfirmationOpen(): Promise<boolean> {
        return this.isVisible(this.page.getByRole("dialog").filter({ hasText: /finish|end|complete/i }));
    }
    /**
    ...
    async isAddExerciseModalOpen(): Promise<boolean> {
        return this.isVisible(this.page.getByRole("dialog").filter({ hasText: /exercise/i }));
    }
    /**
