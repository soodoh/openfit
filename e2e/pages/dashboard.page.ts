...
export class DashboardPage extends BasePage {
    ...
    async isNewSessionModalOpen(): Promise<boolean> {
        return this.isVisible(this.page.getByRole("dialog").filter({ hasText: /new session|workout/i }));
    }
    /**
    ...
    async getRecentSessionsCount(): Promise<number> {
        await this.waitForConvexData();
        const cards = this.page.locator('[class*="group block"], [class*="cursor-pointer"]');
