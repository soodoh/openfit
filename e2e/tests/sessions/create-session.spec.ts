...
e2eTest.describe("Create Session", () => {
    e2eTest("should open new session modal from dashboard", async ({ dashboardPage, page, }) => {
    ...
    e2eTest("should open new session modal from workout page", async ({ workoutPage, page, }) => {
    ...
    e2eTest("should create session from routine template", async ({ dashboardPage, routinesPage, page, }) => {
    ...
    e2eTest("should close new session modal on cancel", async ({ dashboardPage, page, }) => {
        await dashboardPage.goto();
        await dashboardPage.waitForConvexData();
        // Open modal
        await dashboardPage.clickNewSession();
    ...
    e2eTest("should close modal by clicking close button", async ({ dashboardPage, page, }) => {
        await dashboardPage.goto();
        await dashboardPage.waitForConvexData();
        // Open modal
        await dashboardPage.clickNewSession();
