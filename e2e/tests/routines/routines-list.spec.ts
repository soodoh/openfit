...
e2eTest.describe("Routines List", () => {
    ...
    e2eTest("should display search input when routines exist", async ({ routinesPage, }) => {
    ...
    e2eTest("should search routines", async ({ routinesPage }) => {
        ...
        e2eExpect(searchedNames.length !== initialNames.length || hasNoResults).toBe(true);
    });
    ...
    e2eTest("should show empty state when no routines", async ({ routinesPage }) => {
        await routinesPage.waitForConvexData();
        const hasEmpty = await routinesPage.hasEmptyState();
    ...
    e2eTest("should show no results state for search with no matches", async ({ routinesPage, }) => {
        await routinesPage.waitForConvexData();
        const hasEmpty = await routinesPage.hasEmptyState();
        if (hasEmpty) {
            e2eTest.skip();
    ...
    e2eTest("should close modal when clicking cancel or escape", async ({ routinesPage, }) => {
        await routinesPage.waitForConvexData();
        // Open modal
        const hasEmpty = await routinesPage.hasEmptyState();
        if (hasEmpty) {
