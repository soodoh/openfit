...
e2eTest.describe("Routine CRUD", () => {
    ...
    e2eTest("should show routine details when clicking a routine card", async ({ routinesPage, page, }) => {
        await routinesPage.goto();
        await routinesPage.waitForConvexData();
        const hasEmpty = await routinesPage.hasEmptyState();
        if (hasEmpty) {
    ...
    e2eTest("should validate required fields when creating routine", async ({ routinesPage, page, }) => {
        await routinesPage.goto();
        await routinesPage.waitForConvexData();
        // Open create modal
        const hasEmpty = await routinesPage.hasEmptyState();
