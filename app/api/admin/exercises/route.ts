...
export async function GET(request: Request) {
  try {
    ...
    const conditions = search ? like(exercises.name, `%${search}%`) : undefined;
    ...
    const allExercises = await db.query.exercises.findMany({
      with: {
        ...
        instructions: {
          orderBy: [asc(exerciseInstructions.order)],
        },
        images: {
          orderBy: [asc(exerciseImages.order)],
        },
      },
      where: conditions,
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ id: exerciseId });
  } catch (error) {
