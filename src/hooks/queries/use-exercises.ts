...
function buildExerciseQueryString(filters: ExerciseFilters, cursor?: string, limit?: number): string {
...
async function fetchExercises(filters: ExerciseFilters = {}, cursor?: string, limit = 20, signal?: AbortSignal): Promise<PaginatedResponse<Exercise>> {
...
async function searchExercises(term: string, equipmentIds?: string[], limit = 20, signal?: AbortSignal): Promise<Exercise[]> {
    const params = new URLSearchParams();
    if (term) {
        params.set("q", term);
    }
...
async function searchSimilarExercises(primaryMuscleIds: string[], options: {
    search?: string;
    equipmentIds?: string[];
...
export function useExercises(filters: ExerciseFilters = {}): any {
    return useInfiniteQuery({
        ...
        queryFn: ({ pageParam, signal }) => fetchExercises(filters, pageParam, 20, signal),
...
export function useExerciseSearch(term: string, equipmentIds?: string[], limit = 20): any {
    return useQuery({
...
export function useSimilarExercises(primaryMuscleIds: string[] | undefined, options: {
    search?: string;
    equipmentIds?: string[];
    excludeExerciseId?: string;
    limit?: number;
