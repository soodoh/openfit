...
async function fetchPaginatedAdmin<T>(url: string, params: AdminPaginationParams, signal?: AbortSignal): Promise<PaginatedResponse<T>> {
...
async function fetchPaginatedLookup(type: string, params: AdminPaginationParams, signal?: AbortSignal): Promise<PaginatedResponse<LookupItem>> {
    const searchParams = new URLSearchParams({
        type,
        page: String(params.page),
        pageSize: String(params.pageSize),
...
export function useAdminUsersPaginated(params: AdminPaginationParams): any {
    return useQuery({
        ...
        queryFn: ({ signal }) => fetchPaginatedAdmin<UserWithProfile>("/api/admin/users", params, signal),
...
export function useAdminExercisesPaginated(params: AdminPaginationParams): any {
    return useQuery({
        ...
        queryFn: ({ signal }) => fetchPaginatedAdmin<ExerciseWithRelations>("/api/admin/exercises", params, signal),
        placeholderData: keepPreviousData,
    });
}
export function useAdminLookupPaginated(type: string, params: AdminPaginationParams): any {
    const keyFn = {
...
export type { UserWithProfile, ExerciseWithRelations, LookupItem, PaginatedResponse, AdminPaginationParams, };
