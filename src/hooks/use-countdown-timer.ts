...
export const useCountdownTimer = ({ expiryTimestamp, autoStart = true, onExpire, interval = DEFAULT_INTERVAL_MS, }: {
    ...
    const endTimeMsRef = useRef<number | undefined>(autoStart && initialSeconds > 0 ? Date.now() + initialSeconds * 1000 : undefined);
    ...
    const restart = useCallback((nextExpiryTimestamp: Date, nextAutoStart = true) => {
