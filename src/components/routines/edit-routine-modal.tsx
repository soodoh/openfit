...
export const EditRoutineModal = ({ open, onClose, routine, }: {
    routine?: Routine;
    open: boolean;
    onClose: () => void;
}): any => {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="px-6 py-5 space-y-5">
            <div className="space-y-2">
              ...
              <Input id="routine-name" name="name" placeholder="e.g., Push Pull Legs, Full Body, Upper/Lower" required value={name} onChange={(e) => setName(e.target.value)} className={`h-11 ${errors.name ? "border-destructive focus-visible:ring-destructive" : ""}`}/>
              {errors.name && (<p className="text-sm text-destructive flex items-center gap-1.5">
            ...
            <div className="space-y-2">
              ...
              <textarea id="routine-description" name="description" placeholder="Add notes about this routine, goals, or schedule..." rows={3} value={description} onChange={(e) => setDescription(e.target.value)} className={`flex min-h-[80px] w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 resize-none ${errors.description
            ? "border-destructive focus-visible:ring-destructive"
            : ""}`}/>
              {errors.description && (<p className="text-sm text-destructive flex items-center gap-1.5">
                  <AlertCircle className="h-3.5 w-3.5"/>
                  {errors.description}
                </p>)}
            </div>
