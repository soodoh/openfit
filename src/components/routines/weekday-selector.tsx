...
export const WeekdaySelector = ({ selectedWeekdays, onChange, disabled = false, }: {
    ...
    const toggleWeekday = (weekday: number) => {
        ...
        const newWeekdays = selectedWeekdays.includes(weekday)
            ...
            : [...selectedWeekdays, weekday].toSorted((a, b) => a - b);
        onChange(newWeekdays);
    };
    return (<div className="space-y-3">
      {/* Weekday Selector */}
      <div className="flex gap-2 justify-between">
        {weekdayLabels.map((day) => (<button key={`weekday-${day.value}`} type="button" onClick={() => toggleWeekday(day.value)} disabled={disabled} aria-label={day.full} aria-pressed={selectedWeekdays.includes(day.value)} className={`
      ...
      {selectedWeekdays.length > 0 && (<p className="text-xs text-muted-foreground pt-1">
          Selected:{" "}
