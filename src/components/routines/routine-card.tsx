...
export function RoutineCard({ routine, currentSession, }: {
    ...
    return (<>
      <Card onClick={() => setIsModalOpen(true)} className="group flex flex-col h-full overflow-hidden border-border/50 bg-card/50 backdrop-blur-xs shadow-xs hover:shadow-lg hover:border-border transition-all duration-300 cursor-pointer">
      ...
      <RoutineModal open={isModalOpen} onClose={() => setIsModalOpen(false)} routine={routine} currentSession={currentSession}/>
    </>);
}
