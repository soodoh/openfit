import { AutocompleteExercise } from "@/components/exercises/autocomplete-exercise";
...
export const AddExerciseRow = ({ sessionOrDayId, isSession, }: {
    ...
    return (<form onSubmit={handleSubmit} className="flex items-center gap-3">
      <div className="flex-1 min-w-0">
        <AutocompleteExercise value={exercise} onChange={(newExercise) => setExercise(newExercise)}/>
      </div>
      <div className="flex items-center gap-2">
        <div className="w-16">
          <Input type="number" placeholder="Sets" value={numSets} onChange={(event) => setNumSets(event.target.value)} min={1} className="h-9 text-center"/>
        </div>
        <Button type="submit" size="sm" disabled={!exercise} className="gap-1.5">
