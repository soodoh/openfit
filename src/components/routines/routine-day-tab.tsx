...
import { useCreateSession, useRoutineDay, useUnits, useUpdateRoutineDay, } from "@/hooks";
...
export const RoutineDayTab = ({ dayId, currentSession, onDeleted, }: {
    dayId: string;
    ...
    return (<div className="flex flex-col h-full min-h-0">
      ...
      <div className="px-6 py-4 border-b border-border/50 space-y-4">
        <div className="space-y-2">
          ...
          <Input id="day-name" value={description} onChange={(e) => setDescription(e.target.value)} onBlur={handleDescriptionBlur} placeholder="e.g., Chest & Triceps" className="h-10"/>
        </div>
        ...
        <div className="space-y-2">
          ...
          <WeekdaySelector selectedWeekdays={weekdays} onChange={handleWeekdaysChange}/>
        </div>
      </div>

      {/* Workout List */}
      <div className="flex-1 overflow-y-auto px-6 min-h-0">
        <WorkoutList view={ListView.EditTemplate} sessionOrDayId={dayId} setGroups={routineDay.setGroups} units={units}/>
      </div>
      ...
      <div className="px-6 py-4 border-t border-border/50 bg-muted/30 flex items-center justify-between gap-4">
        ...
        <Button onClick={handleStartWorkout} disabled={Boolean(currentSession) || isStarting} className="gap-2">
          {isStarting ? (<>
      ...
      <DeleteDayModal open={isDeleting} onClose={() => setIsDeleting(false)} dayId={dayId} onSuccess={onDeleted}/>
    </div>);
};

export default RoutineDayTab;
