...
const CarouselContext = React.createContext<CarouselContextProps | undefined>(null);
...
const Carousel = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & CarouselProps>(({ orientation = "horizontal", opts, setApi, plugins, className, children, ...props }, ref) => {
    ...
    const contextValue = React.useMemo<CarouselContextProps>(() => ({
        ...
        scrollSnaps,
        scrollTo,
    }), [api, canScrollNext, canScrollPrev, carouselRef, opts, orientation, scrollNext, scrollPrev, scrollSnaps, scrollTo, selectedIndex]);
...
const CarouselDots = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => {
    ...
    return (<div ref={ref} className={cn("flex justify-center gap-1.5 mt-3", className)} {...props}>
      {scrollSnaps.map((snap, index) => (<button key={snap} type="button" onClick={() => scrollTo(index)} className={cn("w-2 h-2 rounded-full transition-colors", index === selectedIndex
                ? "bg-primary"
                : "bg-primary/30 hover:bg-primary/50")} aria-label={`Go to slide ${index + 1}`}/>))}
    </div>);
});
