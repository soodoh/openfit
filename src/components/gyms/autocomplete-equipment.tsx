...
export function AutocompleteEquipment({ selectedIds, onSelect, disabled = false, }: AutocompleteEquipmentProps): any {
    ...
    return (<Popover open={open} onOpenChange={setOpen}>
      <PopoverAnchor asChild>
        <Input ref={inputRef} placeholder={allSelected ? "All equipment added" : "Search equipment..."} value={searchTerm} onChange={(e) => {
            setSearchTerm(e.target.value);
            if (!open) {
                setOpen(true);
            }
        }} onFocus={() => !disabled && !allSelected && setOpen(true)} onBlur={(e) => {
            // Don't close if clicking inside popover
            if (!e.relatedTarget?.closest("[data-radix-popper-content-wrapper]")) {
                setOpen(false);
            }
        }} onKeyDown={handleKeyDown} disabled={disabled || allSelected} className="w-full"/>
      </PopoverAnchor>
      <PopoverContent className="w-(--radix-popover-trigger-width) p-0" align="start" onOpenAutoFocus={(e) => e.preventDefault()} onInteractOutside={(e) => {
        ...
        <Command shouldFilter={false}>
          <CommandList>
            ...
            <CommandGroup>
              {availableEquipment.map((item, index) => (<CommandItem key={item.id} value={item.id} onSelect={() => handleSelect(item.id)} className={`cursor-pointer ${index === 0 ? "bg-accent" : ""}`}>
                  {item.name}
                </CommandItem>))}
            </CommandGroup>
          </CommandList>
