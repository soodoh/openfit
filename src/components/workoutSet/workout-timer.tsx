...
export const WorkoutTimer = ({ set, onComplete, }: {
    ...
    const { isRunning, totalSeconds: remainingSeconds, start, pause, restart, } = useCountdownTimer({
        expiryTimestamp,
        autoStart: false,
    });
    ...
    return (<>
      ...
      <div className="flex items-center">
        <Button variant="ghost" size="icon" onClick={() => setTimerOpen(!isTimerOpen)} className="relative h-10 w-10">
          {!isRunning && remainingSeconds < totalSeconds ? (<Pause className="h-4 w-4"/>) : (<Timer className="h-4 w-4"/>)}
          <ProgressCircle value={percentage} size={40} strokeWidth={2} className="absolute inset-0 text-gray-500"/>
        </Button>
      </div>
    </>);
};
