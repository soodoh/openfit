    ...
    return (<header className="bg-primary text-primary-foreground">
      <div className="mx-auto w-full px-4 sm:px-6 lg:px-8 max-w-(--breakpoint-xl)">
        <nav className="flex items-center py-4">
          <Link to="/">
            <Image src="/logo.svg" alt="OpenFit logo" width={40} height={40} layout="fixed"/>
          </Link>
          <div className="ml-8 hidden flex-1 gap-8 md:flex">
            {navItems.map((item) => (<Link key={item.to} to={item.to} className="text-primary-foreground hover:underline">
                {item.label}
              </Link>))}
          </div>
            ...
            <Sheet>
              ...
              <SheetContent side="left" className="flex flex-col">
                <nav className="flex h-full flex-col items-center justify-center gap-8 text-center">
                  {navItems.map((item) => (<SheetClose asChild key={item.to}>
