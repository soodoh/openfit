...
import { ArrowLeft, Calendar, CheckCircle2, Clock, MessageSquare, Star, } from "lucide-react";
import { Link } from "@tanstack/react-router";
import { EditSessionMenu } from "./edit-session-menu";
export const SessionPage = ({ session, units, }: {
    ...
    return (<div className="min-h-[calc(100vh-4rem)]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="flex items-center gap-3 p-4 rounded-xl bg-card border">
            <div className="w-10 h-10 rounded-lg bg-amber-500/10 flex items-center justify-center">
              <Star className={`h-5 w-5 ${session.impression ? "text-amber-500 fill-amber-500" : "text-muted-foreground"}`}/>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Rating</p>
              {session.impression ? (<div className="flex items-center gap-0.5">
                  {Array.from({ length: 5 }, (_, i) => (<Star key={i} className={`h-3.5 w-3.5 ${i < session.impression!
                    ? "text-amber-400 fill-amber-400"
                    : "text-muted/40"}`}/>))}
                </div>) : (<p className="text-sm font-semibold">—</p>)}
            </div>
          </div>

          {/* Notes Card */}
          {session.notes ? (<div className="flex items-start gap-3 p-4 rounded-xl bg-card border">
            ...
            </div>) : (<div className="flex items-center gap-3 p-4 rounded-xl bg-card border">
      ...
      <div className="mx-auto w-full px-4 sm:px-6 lg:px-8 max-w-(--breakpoint-lg)">
        <WorkoutList view={ListView.ViewSession} sessionOrDayId={session.id} setGroups={session.setGroups} units={units}/>
      </div>
    </div>);
};
