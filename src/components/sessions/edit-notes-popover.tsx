...
export const EditNotesPopover = ({ session, }: {
      ...
      <PopoverTrigger asChild>
        <button type="button" className="flex items-center gap-3 p-3 rounded-xl bg-card border hover:bg-accent/50 transition-colors text-left w-full h-full">
          ...
          <div className="min-w-0">
            ...
            {session.notes ? (<p className="text-sm leading-tight line-clamp-2">
                {session.notes}
              </p>) : (<p className="text-sm font-semibold leading-none">—</p>)}
          </div>
        </button>
      </PopoverTrigger>
