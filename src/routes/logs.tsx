...
function LogsPage() {
    ...
    const { data: sessions, isLoading: sessionsLoading } = useSessionsByDateRange(startOfCalendar.valueOf(), endOfCalendar.valueOf());
    const { data: currentSession } = useCurrentSession();
    const { data: units, isLoading: unitsLoading } = useUnits();
    const isLoading = sessionsLoading || unitsLoading;
    const sessionsThisMonth = sessions?.filter((session) => dayjs(session.startTime).isSame(currentMonth, "month")).length ?? 0;
    return (<div className="min-h-[calc(100vh-4rem)]">
      ...
      <div className="mx-auto w-full px-4 sm:px-6 lg:px-8 max-w-(--breakpoint-xl) py-8">
        ...
        {!isLoading && sessions && units && (<>
            ...
            <MonthlyCalendar currentMonth={currentMonth} sessions={sessions} currentSessionId={currentSession?.id} units={units} onMonthChange={setCurrentMonth}/>
          </>)}
      </div>
    </div>);
}
