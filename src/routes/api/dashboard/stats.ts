        ...
        handlers: {
            GET: async ({ request }) => {
                ...
                const thisWeekResult = await db
                    .select({ count: count() })
                    .from(schema.workoutSessions)
                    .where(and(eq(schema.workoutSessions.userId, session.user.id), gte(schema.workoutSessions.startTime, monday), isNotNull(schema.workoutSessions.endTime)));
                const thisWeekSessions = thisWeekResult[0]?.count || 0;
                // Calculate current streak (consecutive days with workouts)
                // Get all completed session dates
                const sessionDates = await db
                    .select({
                    date: sql<string> `date(${schema.workoutSessions.startTime})`.as("date"),
                })
                    .from(schema.workoutSessions)
                    .where(and(eq(schema.workoutSessions.userId, session.user.id), isNotNull(schema.workoutSessions.endTime)))
