...
async function getSessionWithData(sessionId: string) {
    ...
    const setGroupsWithSets = await Promise.all(setGroups.map(async (group) => {
        ...
        handlers: {
            ...
            GET: async ({ request }) => {
                ...
                if (startDate && endDate) {
                    conditions.push(gte(schema.workoutSessions.startTime, new Date(Number.parseInt(startDate, 10))));
                    conditions.push(lt(schema.workoutSessions.startTime, new Date(Number.parseInt(endDate, 10))));
                }
                ...
                const sessionsWithData = await Promise.all(sessions.map(async (s) => getSessionWithData(s.id)));
