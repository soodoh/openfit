...
async function ensureTargetOwnership(userId: string, sessionId: string | undefined, routineDayId: string | undefined): Promise<boolean> {
    if (sessionId) {
...
async function getNextSetGroupOrder(sessionId: string | undefined, routineDayId: string | undefined): Promise<number> {
...
async function createSetGroupResponse(userId: string, body: CreateSetGroupBody): Promise<Response> {
    const { sessionId, routineDayId, type = "NORMAL", exerciseId, numSets = 3, } = body;
    if (!sessionId && !routineDayId) {
        return Response.json({ error: "Either sessionId or routineDayId is required" }, { status: 400 });
    }
    if (!exerciseId) {
        return Response.json({ error: "exerciseId is required" }, { status: 400 });
    }
    const hasOwnership = await ensureTargetOwnership(userId, sessionId, routineDayId);
    if (!hasOwnership) {
        return Response.json({ error: "Unauthorized" }, { status: 403 });
    }
    const unitIds = await getDefaultUnitIds();
    if (!unitIds) {
        return Response.json({ error: "Units not found - please seed the database first" }, { status: 500 });
    }
        ...
        handlers: {
            ...
            POST: async ({ request }) => {
                ...
                try {
                    ...
                    return Response.json({ error: "Failed to create set group" }, { status: 500 });
                }
            },
        },
    },
