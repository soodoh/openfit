        ...
        handlers: {
            GET: async ({ request }) => {
                try {
                    ...
                    const conditions = search ? like(schema.exercises.name, `%${search}%`) : undefined;
                    ...
                    const formatted = allExercises.map((ex) => ({
                        ...
                        secondaryMuscleIds: ex.secondaryMuscles.map((sm) => sm.muscleGroupId),
            ...
            POST: async ({ request }) => {
                try {
                    ...
                    if (body.primaryMuscleIds?.length) {
                        await db.insert(schema.exercisePrimaryMuscles).values(body.primaryMuscleIds.map((muscleId: string) => ({
                            id: createId(),
                            exerciseId,
                    ...
                    if (body.secondaryMuscleIds?.length) {
                        await db.insert(schema.exerciseSecondaryMuscles).values(body.secondaryMuscleIds.map((muscleId: string) => ({
                            id: createId(),
                            exerciseId,
                            muscleGroupId: muscleId,
                        })));
                    ...
                    if (body.instructions?.length) {
                        await db.insert(schema.exerciseInstructions).values(body.instructions.map((instruction: string, index: number) => ({
                            id: createId(),
                            exerciseId,
                            instruction,
                            order: index,
                    ...
                    if (body.imageUrls?.length) {
                        await db.insert(schema.exerciseImages).values(body.imageUrls.map((imagePath: string, index: number) => ({
                            id: createId(),
                            exerciseId,
                            path: imagePath,
                            order: index,
