...
async function getFirstImageUrl(exerciseId: string): Promise<string | undefined> {
...
async function withFirstImageUrls<T extends {
    id: string;
}>(exercises: T[]): Promise<Array<T & {
    ...
    server: {
        handlers: {
            ...
            GET: async ({ request }) => {
                ...
                const level = searchParams.get("level") as "beginner" | "intermediate" | "expert" | undefined;
                ...
                if (primaryMuscleId) {
                    exercises = exercises.filter((e) => e.primaryMuscles.some((pm) => pm.muscleGroupId === primaryMuscleId));
                }
                ...
                const page = exercisesWithImages.map((e) => Object.assign(e, {
            ...
            POST: async ({ request }) => {
                ...
                try {
                    ...
                    const { name, equipmentId, force, level, mechanic, categoryId, primaryMuscleIds = [], secondaryMuscleIds = [], instructions = [], } = body;
                    if (!name || !categoryId) {
