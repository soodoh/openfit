...
function RoutinesSkeleton() {
    return (<div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {[1, 2, 3].map((i) => (<div key={i} className="h-64 rounded-xl bg-linear-to-br from-muted/50 to-muted/30 animate-pulse"/>))}
    </div>);
}
function EmptyState() {
...
function RoutinesPage() {
    ...
    const { data: routinesData, isLoading: routinesLoading, fetchNextPage, hasNextPage, isFetchingNextPage, } = useRoutines({ search: isSearching ? searchQuery : undefined });
    ...
    return (<div className="min-h-[calc(100vh-4rem)]">
      <div className="border-b border-border/50 bg-linear-to-b from-accent/5 to-transparent">
        <div className="mx-auto w-full px-4 sm:px-6 lg:px-8 max-w-(--breakpoint-xl) py-8">
          ...
          {(routines.length > 0 || isSearching) && (<div className="relative mt-6 max-w-md">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground"/>
              <Input type="text" placeholder="Search routines..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10"/>
            </div>)}
        </div>
      </div>
      ...
      <div className="mx-auto w-full px-4 sm:px-6 lg:px-8 max-w-(--breakpoint-xl) py-8">
        ...
        {!routinesLoading && routines.length === 0 && !isSearching && (<EmptyState />)}

        {!routinesLoading && routines.length === 0 && isSearching && (<div className="flex flex-col items-center justify-center py-16 px-4">
        ...
        {routines.length > 0 && (<p className="text-sm text-muted-foreground mb-6">
        ...
        {routines.length > 0 && (<>
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {routines.map((routine) => (<RoutineCard key={routine.id} routine={routine} currentSession={currentSession}/>))}
            </div>

            {hasNextPage && (<div ref={sentinelRef} className="flex justify-center mt-8">
