...
import { useCurrentSession, useDashboardStats, useRecentSessions, useUnits, } from "@/hooks";
import { createFileRoute, Link, useNavigate } from "@tanstack/react-router";
import { ArrowRight, CalendarDays, Dumbbell, Flame, LayoutGrid, Loader2, Plus, Target, Trophy, } from "lucide-react";
...
function FeatureCard({ href, icon, title, description, }: {
    ...
    return (<Link to={href} className="group block">
...
function EmptyActivity() {
    return (<Card className="border-dashed p-8 flex flex-col items-center justify-center text-center">
...
function StatsLoadingSkeleton() {
    const skeletonIds = ["total", "week", "streak", "routines"];
    return (<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {skeletonIds.map((skeletonId) => (<Card key={`stats-loading-${skeletonId}`} className="p-4 flex items-center gap-3">
...
function RecentActivitySkeleton() {
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {skeletonIds.map((skeletonId) => (<Card key={`activity-loading-${skeletonId}`} className="p-5">
...
function DashboardContent() {
    ...
    const { data: currentSession, isLoading: currentLoading } = useCurrentSession();
    const { data: units, isLoading: unitsLoading } = useUnits();
    const isLoading = statsLoading || recentLoading || currentLoading || unitsLoading;
    let recentActivityContent: React.ReactNode;
    if (isLoading) {
        recentActivityContent = <RecentActivitySkeleton />;
    }
    else if (!recentSessions || recentSessions.length === 0) {
        recentActivityContent = <EmptyActivity />;
    }
    else {
        recentActivityContent = (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {recentSessions.map((session) => (<SessionSummaryCard key={session.id} session={session as unknown as WorkoutSessionWithData} onClick={() => setSelectedSessionId(session.id)} showEditMenu={false}/>))}
      </div>);
    }
    return (<div className="min-h-[calc(100vh-4rem)]">
      ...
      <div className="border-b border-border/50 bg-linear-to-b from-accent/5 to-transparent">
        <div className="mx-auto w-full px-4 sm:px-6 lg:px-8 max-w-(--breakpoint-xl) py-8 md:py-12">
          ...
          {currentSession && (<div className="mb-8">
          ...
          {/* Stats Grid */}
          {isLoading || !stats ? (<StatsLoadingSkeleton />) : (<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard icon={<Trophy className="h-5 w-5"/>} label="Total Sessions" value={stats.totalSessions}/>
              <StatCard icon={<CalendarDays className="h-5 w-5"/>} label="This Week" value={stats.thisWeekSessions}/>
              <StatCard icon={<Flame className="h-5 w-5"/>} label="Day Streak" value={stats.currentStreak}/>
              <StatCard icon={<LayoutGrid className="h-5 w-5"/>} label="Routines" value={stats.totalRoutines}/>
            </div>)}
        </div>
      </div>
      ...
      <div className="mx-auto w-full px-4 sm:px-6 lg:px-8 max-w-(--breakpoint-xl) py-8">
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <FeatureCard href="/routines" icon={<Target className="h-6 w-6"/>} title="Routines" description="Create and manage your workout templates"/>
            <FeatureCard href="/exercises" icon={<Dumbbell className="h-6 w-6"/>} title="Exercises" description="Browse the exercise library"/>
            <FeatureCard href="/logs" icon={<CalendarDays className="h-6 w-6"/>} title="Workout Logs" description="Review your past sessions"/>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            {recentSessions && recentSessions.length > 0 && (<Link to="/logs" className="text-sm text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1">
                View all
      ...
      {units && (<SessionDetailModal sessionId={selectedSessionId} units={units} open={Boolean(selectedSessionId)} onClose={() => setSelectedSessionId(null)}/>)}
    </div>);
}
function Home() {
